...
#define K_V 1.2                 // 赤外線強度の係数
#define K_DELTA 8.5             // 赤外線方角の係数
...
int dribble = 1;          // Input: ドリブル中か(0:違う, 1:そう)
...
int angle(int now, int target) {
  int work = (target - now ) % 360;
  if (work > 180) {
    return work - 360;
  }
  else if (work < -180) {
...
void runDirection(int target) {
  leftMotor  =  ( 30 + 0.1 * angle(cmps, target) );
  rightMotor =  ( 30 - 0.1 * angle(cmps, target) );
}

void shoot2() {
  int goaldir = cmps;
  double cmpsPi = cmps * PI / 180;

  double cosPi = cos(cmpsPi);
  double sinPi = sin(cmpsPi);

  if ( -30 <= goaldir && goaldir <= 30 ) {
    if (us[0] < FW) {
      if (us[1] < FRONTSIDE) {
        runDirection(90);
      }
      else if (us[3] < FRONTSIDE) {
        runDirection(-90);
      }
      else {
        runDirection(0);
  ...
  else if ( -90 <= goaldir && goaldir < -30) {
    if (us[3] < FW) {
      if (us[0] < FRONTSIDE) {      //左前
        runDirection(150);
      }
      else if (us[2] < FRONTSIDE) { //右前
        runDirection(-90);
      }
      else {                        //ゴール前
        runDirection(0);
  ...
  else if ( 30 <= goaldir && goaldir < 90) {
    if (us[1] < FW) {
      if (us[2] < FRONTSIDE) {      //左前
        runDirection(90);
      }
      else if (us[0] < FRONTSIDE) { //右前
        runDirection(-150);
      }
      else {                        //ゴール前
        runDirection(0);
  ...
  else if ( -150 <= goaldir && goaldir < -90) {
    if (us[3] < FW) {
      if (us[0] < FRONTSIDE) {      //左前
        runDirection(120);
      }
      ...
      if (us[0] < FRONTSIDE) {      //左後
        runDirection(150);
      }
      else if (us[2] < FRONTSIDE) { //右後
        runDirection(0);
      }
      else {                        //ゴール後
        runDirection(0);
  ...
  else if ( 90 < goaldir && goaldir <= 150) {
    if (us[1] < FW) {
      if (us[2] < FRONTSIDE) {      //左前
        runDirection(90);
      }
